              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && quickSearchQuery.trim()) {
                    router.push(`/dashboard/search?q=${encodeURIComponent(quickSearchQuery.trim())}`)
                    setQuickSearchQuery('')
                  }
                }}
                placeholder="Quick search feedback..."
                className="w-full px-4 py-2 pl-10 pr-4 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
              />
